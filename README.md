...
*Proper held-out evaluation: 10 same-seed episodes at **fixed difficulty=0.45**, only the cross-episode memory differs. **Baseline** = fresh agents, no memory; **Trained** = fresh agents wired to `pipeline.memory` (the warm `MemoryStore` populated during the 40-episode training run above). Both arms run the same scenarios so the comparison isolates what cross-episode learning bought us — the curriculum is *not* compressing the gap by raising difficulty in the trained arm. Numbers are regenerated by Section 5 each run; commit them to the repo to update this caption.*
